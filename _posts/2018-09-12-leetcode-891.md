...
則我們可以確定每個case的$C_1$項了，也就是 $c(n,1)=B_n=2^{n-1}-1,~\forall~n\ge2$
...
由於$n$的大小可能達到數萬，在$B_i$的計算上可能會遭遇困難，而由於本題要求我們計算結果要mod `1e9+7`，故我們可以使用**modular exponetial**的方法，如下：
先考慮 $(1000)_{10}=(1111101000)_{2}$，則
$$
\begin{align*}
